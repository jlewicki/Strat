...
/// Similar to StateMachineAgent<'D,'M>, defined with C# interop-friendly types.
type IStateMachineAgent<'D,'M> =
   inherit IDisposable 
   abstract Start: [<Optional>] timeout:Nullable<TimeSpan> -> unit
   abstract StartAsync: [<Optional>] timeout:Nullable<TimeSpan> -> Task
   abstract Stop: [<Optional>] timeout:Nullable<TimeSpan> -> unit
   abstract StopAsync: [<Optional>] timeout:Nullable<TimeSpan> -> Task
   abstract SendMessage: message:'M * [<Optional>] timeout:Nullable<TimeSpan> -> StateMachineContext<'D,'M>
   abstract SendMessageAsync: message:'M * [<Optional>] timeout:Nullable<TimeSpan> -> Task<StateMachineContext<'D,'M>>
   abstract Context : StateMachineContext<'D,'M>
   abstract Lifecycle : AgentLifecycle<'D,'M>
   abstract Started : IObservable<StateMachineContext<'D,'M>> 
   abstract Stopped : IObservable<option<StopReason>> 
   abstract MessageProcessed : IObservable<MessageProcessed<'D,'M>>
   abstract Transitioned : IObservable<MessageHandled<'D,'M>>
   abstract Error: IObservable<exn>
...
type StateMachineAgent<'D,'M>
   ...
   interface Strat.StateMachine.Interop.IStateMachineAgent<'D,'M> with
      member this.Start timeout = this.Start (?timeout = Option.ofNullable timeout)
      member this.StartAsync timeout = this.StartAsync (?timeout = Option.ofNullable timeout)
      member this.Stop timeout = this.Stop (?timeout = Option.ofNullable timeout)
      member this.StopAsync timeout = this.StopAsync (?timeout = Option.ofNullable timeout)
      member this.SendMessage (message, timeout) = this.SendMessage (message, ?timeout = Option.ofNullable timeout)
      member this.SendMessageAsync (message, timeout) = this.SendMessageAsync (message, ?timeout = Option.ofNullable timeout)
      member this.Context = this.Context
      member this.Lifecycle = this.Lifecycle
      member this.Started = this.Started
      member this.Stopped = this.Stopped
      member this.MessageProcessed = this.MessageProcessed
      member this.Transitioned = this.Transitioned
